...
function alpha(value, length) {
    const normalized = stripAccents(`${value !== null && value !== void 0 ? value : ''}`)
        .toUpperCase()
        .replace(/[^A-Z0-9 .,&\-/]/g, ' ')
        .slice(0, length);
...
function isPixPayment(payment) {
    var _a, _b, _c;
    return `${(_a = payment.tipoPagamento) !== null && _a !== void 0 ? _a : ''}`.trim().toUpperCase() === 'PIX'
        || Boolean(`${(_b = payment.tipoChavePix) !== null && _b !== void 0 ? _b : ''}`.trim())
        || Boolean(`${(_c = payment.chavePix) !== null && _c !== void 0 ? _c : ''}`.trim());
}
...
function inferTipoChavePix(chavePix) {
    const chave = `${chavePix !== null && chavePix !== void 0 ? chavePix : ''}`.trim();
...
function normalizeTipoChavePix(value, chavePix) {
    const normalized = `${value !== null && value !== void 0 ? value : ''}`.trim().toUpperCase();
    if (!normalized) {
...
function getJsonValue(item, path, fallback = '') {
    ...
    return (_a = path.split('.').reduce((value, key) => {
        if (value && typeof value === 'object' && key in value) {
            return value[key];
        }
        return undefined;
    }, item)) !== null && _a !== void 0 ? _a : fallback;
}
