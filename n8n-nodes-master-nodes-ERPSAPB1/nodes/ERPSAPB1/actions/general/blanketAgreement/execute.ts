...
import { IBlanketAgreementOptions } from '../../../transport/Interfaces';
import { parseSelectedFields } from '../../../utils/select';

export async function blanketAgreement(this: IExecuteFunctions, api: ERPSAPB1Api, index: number): Promise<INodeExecutionData[]> {
    const filters = this.getNodeParameter('filters', index, {}) as IBlanketAgreementOptions,
    ...
    const blanketAgreements = await api.listBlanketAgreements(filters, limitPagination ? maxPages : undefined, selectedFields);

    if (originMetadata.length > 0)  {
        return this.helpers.returnJsonArray({originMetadata, blanketAgreements});
    }

    return this.helpers.returnJsonArray(blanketAgreements);
}
