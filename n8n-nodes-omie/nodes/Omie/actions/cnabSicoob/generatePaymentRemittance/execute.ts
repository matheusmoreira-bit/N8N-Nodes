...
function requireValue(value: string, fieldName: string, itemIndex: number): string {
    if (!value) {
...
function withSupplierPaths(paths: string[]): string[] {
...
function isObjectEmpty(value: IDataObject): boolean {
    return Object.keys(value).length === 0;
}

function extractPayableJson(inputItems: INodeExecutionData[]): IDataObject[] {
    const payables: IDataObject[] = [];
    const arrayKeys = [
    ...
    for (const item of inputItems) {
        ...
        for (const key of arrayKeys) {
            ...
            if (Array.isArray(value)) {
                payables.push(...value.filter((entry): entry is IDataObject => (
                    Boolean(entry)
                    && typeof entry === 'object'
                    && !Array.isArray(entry)
                    && !isObjectEmpty(entry as IDataObject)
                )));
                expanded = true;
                break;
            }
...
function hasBarcodeData(payable: IDataObject): boolean {
    return Boolean(normalizeBarcode(getBarcodeValue(payable)));
}

function hasTransferBankData(payable: IDataObject): boolean {
    return Boolean(
        toStringValue(getFirstJsonValue(payable, fieldPaths.codigoBancoFavorecido))
...
function hasFavoredIdentityData(payable: IDataObject): boolean {
    return Boolean(
        toStringValue(getFirstJsonValue(payable, fieldPaths.nomeFavorecido))
...
function mergeSupplierDetails(payable: IDataObject, supplier: IDataObject): IDataObject {
...
async function enrichPayablesWithOmieDetails(
    api: OmieApi,
    payables: IDataObject[],
    warnings: IDataObject[],
    fetchDetailsFromOmie: boolean,
): Promise<IDataObject[]> {
    if (!fetchDetailsFromOmie) {
        return payables;
    }

    const enrichedPayables: IDataObject[] = [];
    const supplierCache = new Map<string, IDataObject>();
        ...
        if (Object.keys(params).length === 0) {
            ...
            try {
                ...
                warn(
                    ...
                    index,
                    'Consulta Omie',
                    `Não foi possível consultar detalhes da conta a pagar: ${(error as Error).message}`,
                );
            }
        }
            ...
            if (supplierCacheKey) {
                try {
                    ...
                    warn(
                        warnings,
                        index,
                        'Consulta Fornecedor Omie',
                        `Não foi possível consultar dados bancários do fornecedor: ${(error as Error).message}`,
                    );
                }
            }
        }
...
export async function execute(this: IExecuteFunctions, api: OmieApi): Promise<INodeExecutionData[]> {
    const inputItems = this.getInputData();
    const payableItems = extractPayableJson(inputItems);

    if (payableItems.length === 0) {
        throw new Error(
            'Nenhum contas a pagar foi recebido para gerar o CNAB. '
    ...
    const fetchDetailsFromOmie = this.getNodeParameter('fetchDetailsFromOmie', 0, true) as boolean;
    ...
    const payments: Cnab240SicoobPaymentData[] = [];
    ...
    for (const [index, json] of enrichedPayableItems.entries()) {
        ...
        if (valor <= 0) {
            ...
            if (ignorePaymentErrors) {
                warn(
                    ...
                    index,
                    'Valor do Pagamento',
                    `Valor inválido ou ausente ('${toStringValue(rawValor) || 'vazio'}'). Gerado com 0. Campos disponíveis: ${availableKeys || 'nenhum'}.`,
                );
            ...
            throw new Error(
                `Valor de pagamento inválido no item ${index + 1}. `
            ...
            if (codigoBarras.length !== 44) {
                if (ignorePaymentErrors) {
                    warn(
                        ...
                        index,
                        'Código de Barras',
                        `Código de barras/linha digitável inválido (${codigoBarras.length} dígitos). Pagamento ignorado.`,
                    );
                    skippedPayments.push({
                        item: index + 1,
                ...
                throw new Error(
                    `Código de barras/linha digitável inválido no item ${index + 1}. `
        ...
        if (missingTransferFields.length > 0) {
            ...
            throw new Error(
                `Não foi possível identificar a forma de pagamento no item ${index + 1}. `
    ...
    if (payments.length === 0) {
        throw new Error(
            'Nenhum pagamento válido foi encontrado para gerar a remessa. '
